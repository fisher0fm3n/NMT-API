...
function ensureLibrarySchema(withClient) {
  if (!schemaPromise) {
    schemaPromise = withClient(async (db) => {
      ...
      // One vote per reader per poll. Signed-in readers vote under their
      // email; everyone else under an anonymous id their browser keeps, so a
      // reader can change their mind but never count twice.
      await db.query(`
        CREATE TABLE IF NOT EXISTS public.library_poll_votes (
          id          BIGSERIAL PRIMARY KEY,
      ...
      `);
      await db.query(
        `CREATE INDEX IF NOT EXISTS library_poll_votes_poll_idx
           ON public.library_poll_votes (poll_id, option_id)`,
      );
...
/** How every option of a poll has polled: `{ counts: {option_id: n}, total }`. */
async function readPoll(db, pollId) {
  const { rows } = await db.query(
    `SELECT option_id, COUNT(*)::int AS n
       FROM public.library_poll_votes
      WHERE poll_id = $1
      GROUP BY option_id`,
    [pollId],
  );
  ...
  let total = 0;
  for (const r of rows) {
    counts[r.option_id] = r.n;
    total += r.n;
  }
  return { poll_id: pollId, counts, total };
}

/** Records (or changes) one voter's answer, then returns the fresh tally. */
async function castVote(db, pollId, voter, optionId) {
  await db.query(
    `INSERT INTO public.library_poll_votes (poll_id, voter, option_id)
     VALUES ($1, $2, $3)
     ON CONFLICT (poll_id, voter) DO UPDATE
       SET option_id = EXCLUDED.option_id, updated_at = now()`,
    [pollId, voter, optionId],
  );
  return { ...(await readPoll(db, pollId)), choice: optionId };
}
